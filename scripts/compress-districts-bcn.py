#!/usr/bin/env python3
"""
Compress Barcelona district GeoJSON files by:
1. Shortening property names to match barcelonatrees format
...
PROPERTY_MAP = {
    "species": "sn",              # scientific name
    "common_name": "cn",          # common name
    "NBRE_DTO": "dt",             # district
    "NBRE_BARRI": "nb",           # neighborhood
    "CODIGO_ESP": None,           # Remove - species code not needed
    "diameter": "d",              # diameter in cm
    "height": "h"                 # height in m
}
...
def main():
    ...
    # Get all district GeoJSON files (skip Barcelona ones if they exist)
    all_files = sorted(districts_dir.glob('district_*.geojson'))
    
    # Filter to only Barcelona districts (codes 01-10 and 00)
    barcelona_districts = ['00', '01', '02', '03', '04', '05', '06', '07', '08', '09', '10']
    geojson_files = []
    
    for file in all_files:
        # Extract district code from filename (e.g., district_02_EIXAMPLE.geojson -> 02)
        parts = file.stem.split('_')
        if len(parts) >= 2:
            code = parts[1]
            # Only process Barcelona districts
            if code in barcelona_districts:
                # Check if it's a Barcelona district by name
                if any(bcn_name in file.stem.upper() for bcn_name in 
                       ['CIUTAT_VELLA', 'EIXAMPLE', 'GRÀCIA', 'HORTA', 'LES_CORTS', 
                        'NOU_BARRIS', 'SANT_ANDREU', 'SANT_MARTÍ', 'SANTS', 'SARRIÀ']):
                    geojson_files.append(file)
    
    if not geojson_files:
        print("No Barcelona district GeoJSON files found!")
        print("Looking for files with Barcelona district names...")
        return
    
    print(f"Found {len(geojson_files)} Barcelona district files to compress\n")
